...
#figure(
  ...
  caption: [Trial-level agreement between markerless and marker-based reference center of mass path length (n = 12) per tracker (Top: Mediapipe; Middle: ViTPose; Bottom: RTMPose). Left: Identity plot with identity line (dashed) and line of best fit (red). Right: Bland-Altman plot of differences between tracker-derived and reference estimates plotted against the mean of the two measurements. Dashed lines represent bias, and dotted lines represent 95% limits of agreement. Colors indicate balance condition.]) <fig-agreement-all>
