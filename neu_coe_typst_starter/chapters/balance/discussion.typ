...
Accordingly, one limitation of this study is lack of center of pressure, which is most often used in posturography, to use as a reference, as behavior between COM and COP in quiet standing can vary @panzerBiomechanicalAssessmentQuiet1995. However, the NIH Toolbox uses an implementation of the CTSIB-M called the Standing Balance Test, which uses an accelerometer worn as a waist to detect postural sway, calculating path length metrics from that @StandingBalanceTest, which has also been validated in use for balance studies @pellerValidityReliabilityNIH2023. 
