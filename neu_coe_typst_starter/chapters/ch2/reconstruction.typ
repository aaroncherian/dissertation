...
#figure(
  image("reconstructed_human.png", width:40%),
...
=== SkellyForge
`SkellyForge` is a separate post-processing package that operates on the reconstructed 3D trajectories. It applies gap interpolation for frames where no acceptable triangulation solution was found, followed by low-pass Butterworth filtering to attenuate high-frequency noise in the coordinate time series. Like our pose estimation manager, SkellyTracker, this software is designed as a modular component of the pipeline, with an architecture that can accommodate additional post-processing methods as they are developed.
