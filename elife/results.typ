...
#figure(
  ...
  caption: [Overview of the study design and validation analyses. *A.* Markerless and marker-based motion capture data were acquired simultaneously using six consumer webcams and a reference marker-based system. Participants completed two movement assessments: treadmill walking across increasing speeds to evaluate dynamic gait performance and the Modified Clinical Test of Sensory Interaction on Balance (CTSIB-M) to evaluate subtle postural adjustments under progressively more challenging sensory conditions. *B.* Markerless data were reconstructed using three pose estimation backends: MediaPipe, ViTPose, and RTMPose, and compared with the marker-based reference. *C.* Gait validity was evaluated across increasing walking speeds using 3D joint trajectories, joint kinematics, and spatiotemporal gait parameters. Analyses included error metrics, statistical parametric mapping, Bland-Altman analysis, and intraclass correlation coefficients. *D.* Balance validity was evaluated during the CTSIB-M using whole-body center-of-mass trajectories, path length, 95% confidence ellipse area, and mean horizontal velocity. Analyses assessed both agreement with the marker-based reference and sensitivity to visual, proprioceptive, and combined sensory perturbations.]
) <fig-overview>
...
We next evaluated whether the same markerless reconstructions captured differences in postural stability during standing balance tasks. Participants completed two trials of the Modified Clinical Test of Sensory Interaction on Balance (CTSIB-M). During each trial, participants stood for 60 seconds under four conditions that varied visual input and support surface stability: 1) eyes open on a firm surface; 2) eyes closed on a firm surface; 3) eyes open on a foam surface; and 4) eyes closed on a foam surface. Center of mass (COM) trajectories were estimated from each set of pose-estimated derived 3D data and from the marker-based reference data. Postural stability was then quantified using COM path length, 95% confidence ellipse area, and mean 2D COM velocity. Representative COM trajectories projected onto the ground plane for each CTSIB-M condition are shown in @fig-xy-plane.
...
#figvideo(
  ...
  short: [Reconstructed postural sway across the four CTSIB-M conditions for a representative participant trial.],
...
[*PLACEHOLDER: Maybe a video of all 3 fmc skeletons in their original alignment conditions? makes it very obvious where ViTPose is scaled larger*]